...
function runAllEnterpriseTests() {
  ...
  const tests = [
    ...
    testKruczaLocationMapping430_,
    testGenericBeerClassification430_,
    testInventoryHeaderDetection430_,
...
function testParserLocations_() {
  ...
  const parsed = parseInventoryText(
    'bar Litowel Pomelo 30 Litowel Pomelo 2 zaplecze Jurajska Pomarancza 120',
    context
  );
  ...
  assertCondition_(
    parsed[0].location === 'bar' &&
    parsed[1].location === 'bar' &&
    parsed[2].location === 'zaplecze',
...
function testLocationPrefixProductNameIntegrity_() {
  ...
  const parsed = parseInventoryText('bar Inne beczki Pilsner 2', context);
  ...
  assertCondition_(parsed[0].value === 2 && parsed[0].location === 'bar',
  ...
  const exact = parseInventoryText('bar Inne beczki Pilsner 2', exactContext);
...
function testBusinessCategoryNormalization_() {
  ...
  assertCondition_(normalizeBusinessCategory_('PIWO') === 'PIWO',
    'Kategoria PIWO nie zostala znormalizowana.');
}
...
function testSummaryColumnMapping284_() {
  assertCondition_(getInventorySummaryLayout_('NORMAL').finalTotal === 'I', 'NORMAL musi korzystać z kolumny I.');
  assertCondition_(getInventorySummaryLayout_('KEG').finalTotal === 'I', 'KEG musi korzystać z kolumny I.');
  assertCondition_(getInventorySummaryLayout_('LOCATION').finalTotal === 'D', 'LOCATION musi korzystać z kolumny D.');
}
...
function testSummaryInputColumns284_() {
  ...
  assertCondition_(normal.grossWeight === 'B' && normal.fullUnits === 'F', 'Nieprawidłowe kolumny wejściowe NORMAL.');
  assertCondition_(keg.grossWeight === 'B' && keg.fullUnits === 'F', 'Nieprawidłowe kolumny wejściowe KEG.');
  assertCondition_(location.warehouse === 'B' && location.darkroom === 'C' && !location.fridges && location.finalTotal === 'D', 'Nieprawidłowe kolumny LOCATION.');
}
...
function testZeroProtectedProductNames292_() {
  ...
  const cola = parseInventoryText('bar fritz kola zero 4 bar fritz jablko 7', context);
...
/** Inventory PRO 4.3 — kontrakt układu Kruczej. */
function testKruczaLocationMapping430_() {
  const product = {
    type: CONFIG.PRODUCT_TYPES.LOCATION,
    category: 'SOFTY',
    columns: getInputColumnsForProductType_(CONFIG.PRODUCT_TYPES.LOCATION)
  };
  assertCondition_(resolveTargetColumn_(product, 12, 'bar') === 'B', 'Bar musi zapisywać się do B.');
  assertCondition_(resolveTargetColumn_(product, 12, 'zaplecze') === 'C', 'Zaplecze musi zapisywać się do C.');
  assertCondition_(resolveTargetColumn_(product, 12, 'magazyn') === 'C', 'Historyczny alias magazyn musi wskazywać Zaplecze.');
  assertCondition_(resolveTargetColumn_(product, 12, 'lodowki') === '', 'Nieznana lokalizacja nie może zostać zapisana do przypadkowej kolumny.');
}

function testGenericBeerClassification430_() {
  const bottleRow = ['', '', '', '', '', '', '0,5', '', ''];
  const kegRow = ['', '', '', '', '', '', '30', '', ''];
  assertCondition_(inferInventoryProductType_('NORMAL', 'PIWO', 'Birra 0,5', bottleRow) === 'NORMAL', 'Piwo butelkowe nie może być KEG.');
  assertCondition_(inferInventoryProductType_('NORMAL', 'PIWO', 'BIERE BLANCHE/WITBIER', kegRow) === 'KEG', 'Pojemność 30 l musi rozpoznać KEG.');
  assertCondition_(inferInventoryProductType_('NORMAL', 'PREMIXY', 'NEROLI KEG', bottleRow) === 'KEG', 'Sufiks KEG musi wymusić typ KEG.');
}

function testInventoryHeaderDetection430_() {
  const header = ['', 'WAGA/SZT W BUTELCE / KEGU', 'WAGA BUTELKI', 'WAGA BEZ BTLK', 'WAGA PREP netto', 'PEŁNE BTLK szt', 'POJEMNOŚĆ'];
  const normal = detectInventoryInputColumnsFromHeaderRow_(header, 'NORMAL', getInputColumnsForProductType_('NORMAL'));
  const location = detectInventoryInputColumnsFromHeaderRow_(['SOFTY na szt.', 'BAR', 'ZAPLECZE', 'RAZEM'], 'LOCATION', getInputColumnsForProductType_('LOCATION'));
  assertCondition_(normal.weight === 'B' && normal.quantity === 'F', 'Nagłówek główny powinien wykryć B/F.');
  assertCondition_(location.warehouse === 'B' && location.darkroom === 'C' && !location.fridges, 'Nagłówek SOFTY powinien wykryć Bar/Zaplecze.');
}

function testSparseWritePlan430_() {
  const plan = buildSparseWritePlan_([
    { saved:true, row:10, column:'B', previousValue:'', newValue:2 },
    { saved:true, row:10, column:'B', previousValue:2, newValue:5 },
    { saved:true, row:11, column:'C', previousValue:1, newValue:4 },
  ...
  const b10 = plan.filter(item => item.a1 === 'B10')[0];
  assertCondition_(b10 && b10.previousValue === '' && b10.newValue === 5, 'Duplikaty B10 muszą zachować pierwszy stan i ostatnią sumę.');
}

function testStandaloneBarParser430_() {
  const context = createParserTestContext_(['BOMBILLA 0,3L']);
  const parsed = parseInventoryText('bar\nBombilla 12', context);
  assertCondition_(parsed.length === 1 && parsed[0].location === 'bar' && parsed[0].value === 12,
    'Samodzielny nagłówek bar nie może konsumować pierwszego produktu.');
}


function testSparseRollback430_() {
  const values = { B10: 5, C10: 2, D10: '' };
  const sheet = {
  ...
  rollbackSparseWritePlan_(sheet, [
    { a1:'B10', previousValue:1, newValue:5 },
    { a1:'C10', previousValue:2, newValue:6 },
    { a1:'D10', previousValue:'', newValue:3 }
  ]);
  assertCondition_(values.B10 === 1, 'Częściowo zapisana komórka B10 musi zostać cofnięta.');
  assertCondition_(values.C10 === 2, 'Komórka jeszcze niezapisana nie może zostać zmieniona przez rollback.');
  assertCondition_(values.D10 === '', 'Pusta komórka jeszcze niezapisana musi pozostać pusta.');
}
