...
function testReportingSummaryColumns290_() {
  if (getInventorySummaryLayout_('NORMAL').finalTotal !== 'I') throw new Error('NORMAL musi czytać I.');
  if (getInventorySummaryLayout_('KEG').finalTotal !== 'I') throw new Error('KEG musi czytać I.');
  if (getInventorySummaryLayout_('LOCATION').finalTotal !== 'D') throw new Error('LOCATION musi czytać D.');
}
...
function testReportingAllowedCategories290_() {
  ...
  ['WINO', 'BITTER', 'PIWO', 'SOFTY'].forEach(category => {
